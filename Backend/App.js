...
const port = process.env.PORT ? process.env.PORT : 8080;//3000
...
//PUT, modifica categoria (Editado el 1/3/2021 para el tp final) ******************************************

//ATENCION: ESTO ES EXPERIMENTAL
...
app.put('/categoria/:id', async (req, res) => {
    ...
    try {
        ...
        if (!req.params.id || !req.body.nombre) {
            res.status(413).send("Es necesario indicar el número ID de la categoría y el nombre"); 
        }

        else {
            let queryId = 'SELECT * FROM categorias WHERE id = ?';
            let resID = await qy(queryId, [req.params.id]);
            
            if (resID.length > 0)
            {
                let searchEqual = 'SELECT id FROM categorias WHERE nombre = ?';//AND id <=> ? , req.params.id
                let resCategoria = await qy(searchEqual, [req.body.nombre.toUpperCase()]);
    
                if (resCategoria.length > 0) {
                    res.status(413).send("Ya existe una categoria con ese nombre");
                }

                let sqlquery = 'UPDATE categorias SET nombre = ? where id = ?';
                let respuesta = await qy(sqlquery, [req.body.nombre.toUpperCase(), req.params.id]);
                res.status(200).send({"Registro": respuesta});
            }
            res.status(413).send("El ID ingresado no es válido");
        }
    }
    catch(ex){
        console.log(ex);
